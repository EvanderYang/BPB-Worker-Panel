...
export function initializeParams(request, env) {
    ...
    if (pathName !== `/${globalThis.subPath}/secrets`) {
        if (!userID || !globalThis.TRPassword) throw new Error(`Please set UUID and ${atob('VHJvamFu')} password first. Please visit <a href="https://${hostName}/${globalThis.subPath}/secrets" target="_blank">here</a> to generate them.`, { cause: "init"});
